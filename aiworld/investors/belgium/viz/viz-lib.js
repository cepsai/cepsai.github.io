  ...
  function sankey(id, links, opts) {
    ...
    links.forEach(l => {
      ...
      if (h > 13) {
        ...
        // with a single target node every source's outflow share is 100% — skip the noise
        if (!opts.noOutflowLabels) flowLabel(nodeW + 6, nudge(y0a + h / 2 + 3.5, lMid, y0a + 10, y0b - 3), 'start', fmtPct(val(l) / srcTot[l.source]));
