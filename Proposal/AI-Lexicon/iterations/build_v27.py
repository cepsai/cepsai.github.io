"""build_v27.py — Digital AI Lexicon v27.

Lands on top of v26. v27 swaps in cross-checked authoritative copy from
`Cross-checked_AI terminology and taxonomy_analysis.xlsx` (sheets:
`About the Digital AI Lexicon`, `Methodology`) onto three pages:
...
v27 operates as a pure HTML post-process on v26:
    1. Read v26 HTML.
    2. Read xlsx sheets via openpyxl.
    3. Perform precise `html.replace()` swaps with idempotency guards.
    4. Write digital_lexicon_v27.html and mirror to ../final_tool.html
...
HTML_V27      = HERE / "digital_lexicon_v27.html"
...
# --------------------------------------------------------------------------- #
# v26 anchor strings -> v27 replacement strings                               #
# --------------------------------------------------------------------------- #
...
def _replace_card(html: str, title_substr: str, row: dict) -> str:
    ...
    if title_pos == -1:
        raise RuntimeError(
            f"build_v27: card title not found: {title_substr!r}"
        )
    # Walk back to the <article class="law-card-v2"...> open tag.
    art_open = html.rfind("<article class=\"law-card-v2", 0, title_pos)
    if art_open == -1:
        raise RuntimeError(
            f"build_v27: <article> open not found for {title_substr!r}"
        )
    art_close = html.find("</article>", title_pos)
    if art_close == -1:
        raise RuntimeError(
            f"build_v27: </article> close not found for {title_substr!r}"
        )
    ...
    if desc_open == -1:
        raise RuntimeError(
            f"build_v27: <p class=\"law-card-v2-desc\"> not found in card "
    ...
    if row["date"] is not None:
        ...
        if eff_idx == -1:
            raise RuntimeError(
                f"build_v27: <strong>Effective:</strong> not found in card "
                f"for {title_substr!r}"
            )
...
def _replace_step_body(html: str, num: int, step_text: str,
    ...
    if body_div_close == -1 or body_div_close < after_h2:
        raise RuntimeError(f"build_v27: step-{num} body </div> not found")
...
def main() -> None:
    ...
    if HOME_TAGLINE_OLD in html:
        ...
        raise RuntimeError("build_v27: home tagline anchor not found")
    ...
    for title_substr, row_idx in CARD_ANCHORS:
        ...
        title_pos = html.find(title_substr)
        if title_pos == -1:
            raise RuntimeError(f"build_v27: card not found: {title_substr!r}")
        art_close = html.find("</article>", title_pos)
    ...
    HTML_V27.write_text(html, encoding="utf-8")
    shutil.copy2(HTML_V27, FINAL_TOOL)
    shutil.copy2(HTML_V27, FINAL_LEXICON)
    print(f"\n  wrote {HTML_V27.name}        ({len(html):,} bytes)")
