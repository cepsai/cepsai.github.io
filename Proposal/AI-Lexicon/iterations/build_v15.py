...
def build_sub_concepts_v15(concept: dict, tab_analyses: list, jid_order: list[str],
    ...
    if ana_card and ana_card.get("rows"):
        # Dimensions where the *full* concept-wide legal text genuinely
        # applies — the analysis-sheet row is a conceptual re-phrasing of
        # the whole definition (Term / Definition / Scope / Exemptions).
        # For any other dim (Risk management, Copyright, Incident
        # reporting, etc.) the concept-wide fallback is wrong: each cell
        # should only show the verbatim from the matching legal row, or
        # nothing at all, so identical "Frontier developer" text doesn't
        # repeat under every unrelated dim.
        _FULL_VERBATIM_DIMS = {
...
def collect_rich_notes(wb_rt, sheet_name: str) -> list[dict]:
    ...
    arrays of {t, b} runs. Each note carries the sub-concept title (the
    bold label one row above the header row) and the dimension label
    (column A of the note's row), so the caller can attribute it to the
    correct sub-concept sub-tab in the HTML. Output:
        [{sub_concept_title, dim_label, title, body_runs}].
    Deduped by (sub_concept_title, body_plain_text)."""
    ...
    # Also capture the sub-concept title from the row above each header row.
    notes_locs: list[tuple[int, int, int, str]] = []  # (header_row, notes_col, end_row, title)
    header_rows: list[tuple[int, int, str]] = []
    for r in range(1, max_row + 1):
        ...
        if jr_count >= 2 and notes_col is not None:
            sub_title = clean(ws.cell(r - 1, 1).value) if r > 1 else ""
            header_rows.append((r, notes_col, sub_title or ""))
    for i, (hr, nc, title) in enumerate(header_rows):
        end_r = (header_rows[i + 1][0] - 2) if i + 1 < len(header_rows) else max_row
        notes_locs.append((hr, nc, end_r, title))

    notes_out: list[dict] = []
    seen_keys: set[tuple[str, str]] = set()
    for (hr, nc, end_r, sub_title) in notes_locs:
        for r in range(hr + 1, end_r + 1):
            ...
            if key in seen_keys:
                continue
            seen_keys.add(key)
            dim_label = clean(ws.cell(r, 1).value) or ""
            # Title = first non-empty bold run, or the row's dim label, or a generic header.
            title = ""
            for r0 in runs:
                if r0["b"] and r0["t"].strip():
                    ...
                    break
            if not title:
                title = dim_label or "Note"
            notes_out.append({
                "sub_concept_title": sub_title,
                "dim_label":         dim_label,
                "title":             title,
                "body_runs":         runs,
            })
...
def main():
    ...
    for fam in MATRIX_FAMILIES:
        ...
        # Attach interpretative notes to the sub_concept they came from.
        # collect_rich_notes emits each note with the xlsx block's title
        # (the bold label above its header row, e.g. "Provider of high-risk
        # AI systems / developer of high-risk AI systems"). Match each
        # sub_concept to the xlsx block that mentions its title most
        # exclusively — the "/"-separated block titles overlap, so we
        # reserve each xlsx block for one sub_concept.
        def _norm(s: str) -> str:
            s = re.sub(r"gpai", "general purpose ai", (s or "").lower())
            s = re.sub(r"general[- ]purpose\s+ai", "general purpose ai", s)
            s = re.sub(r"ai\s+models?", "ai model", s)
            s = re.sub(r"ai\s+systems?", "ai system", s)
            s = re.sub(r"[^a-z0-9 ]+", " ", s)
            return re.sub(r"\s+", " ", s).strip()

        notes_by_title: dict[str, list] = {}
        for n in rich_notes:
            notes_by_title.setdefault(_norm(n.get("sub_concept_title", "")), []).append(n)

        def _score(sc_key: str, xlsx_key: str) -> int:
            if not sc_key or not xlsx_key:
                return 0
            if sc_key == xlsx_key:
                return 1000 + len(sc_key)
            if sc_key in xlsx_key or xlsx_key in sc_key:
                # Prefer the match with more overlapping words — "Deployer"
                # substring-matches both "Deployer" and "Deployer of high-risk
                # AI systems", so word-overlap breaks the tie.
                kw, scw = set(xlsx_key.split()), set(sc_key.split())
                return 500 + len(kw & scw) * 20 + min(len(sc_key), len(xlsx_key))
            kw, scw = set(xlsx_key.split()), set(sc_key.split())
            overlap = len(kw & scw)
            return overlap * 10 if overlap >= 2 else 0

        # All-pairs score, then greedy-assign (highest score first), and if
        # a sub_concept's first choice is already claimed, fall back to its
        # next-best still-free xlsx block.
        all_pairs: list[tuple[int, int, str]] = []  # (-score, sc_idx, xlsx_key)
        for i, sc in enumerate(sub_concepts):
            sc_key = _norm(sc.get("title", ""))
            for xkey in notes_by_title:
                if not xkey:
                    continue
                sc_score = _score(sc_key, xkey)
                if sc_score > 0:
                    all_pairs.append((-sc_score, i, xkey))
        all_pairs.sort()

        claimed_xkey: dict[str, int] = {}  # xlsx_key -> sc_idx
        claimed_sc: set[int] = set()
        for neg_score, i, xkey in all_pairs:
            if i in claimed_sc or xkey in claimed_xkey:
                continue
            claimed_xkey[xkey] = i
            claimed_sc.add(i)

        # Untitled blocks (xlsx_key == "") come from analysis sheets with no
        # bold sub-block header (single-block sheets: Risk, Incident,
        # Substantial modification, High-risk AI system, GPAI model/system).
        # Assign them round-robin to any sub_concept that hasn't already been
        # claimed — typically there's exactly one sub_concept per such sheet
        # within its family, so this is unambiguous.
        untitled_bucket = notes_by_title.get("", [])
        if untitled_bucket:
            # Group untitled notes by the analysis-sheet's topical cue from
            # the first note's plain text — but simpler: these notes belong
            # to this family's remaining sub_concepts that don't yet have any.
            remaining = [i for i in range(len(sub_concepts)) if i not in claimed_sc]
            if remaining and len(untitled_bucket) == 1 and len(remaining) == 1:
                # Exactly one untitled note, one unclaimed sub_concept — pair.
                claimed_sc.add(remaining[0])
                sub_concepts[remaining[0]].setdefault("_untitled_notes", []).extend(
                    untitled_bucket)
            elif remaining:
                # Multiple untitled notes, multiple unclaimed sub_concepts:
                # score each (note, sc) pair by how many significant words
                # of the sc title appear in the note's plaintext, then
                # greedily assign highest scores first without reusing a
                # sub_concept.
                pairs = []
                for ni, n in enumerate(untitled_bucket):
                    plain = "".join(r["t"] for r in n["body_runs"]).lower()
                    for i in remaining:
                        sc_words = _norm(sub_concepts[i].get("title", "")).split()
                        hits = sum(1 for w in sc_words if len(w) > 3 and w in plain)
                        pairs.append((-hits, ni, i))
                pairs.sort()
                used_notes: set[int] = set()
                used_sc: set[int] = set()
                for neg, ni, i in pairs:
                    if ni in used_notes or i in used_sc:
                        continue
                    used_notes.add(ni)
                    used_sc.add(i)
                    sub_concepts[i].setdefault("_untitled_notes", []).append(
                        untitled_bucket[ni])

        for i, sc in enumerate(sub_concepts):
            xkey = next((k for k, v in claimed_xkey.items() if v == i), None)
            bucket = list(notes_by_title.get(xkey, [])) if xkey else []
            bucket.extend(sc.pop("_untitled_notes", []))
            sc["ceps_notes_rich"] = [
                {
                    "title":     n["title"],
                    "body_runs": n["body_runs"],
                    "dim_label": n.get("dim_label", ""),
                }
                for n in bucket
            ]

        # Keep a family-level pool too for the coverage test, but empty by
        # default so the renderer won't display duplicates on the Overview.
        concepts_v15.append({
            "id":              tab_id,
    ...
    print(f"  rich notes:         {sum(len(sc.get('ceps_notes_rich',[])) for c in concepts_v15 for sc in c.get('sub_concepts',[]))} (per-sub-concept)")
