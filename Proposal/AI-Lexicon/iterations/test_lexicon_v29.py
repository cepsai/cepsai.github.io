"""test_lexicon_v29.py — acceptance tests for v29.

v29 starts as a byte-equivalent scaffold of v27. It reuses the same
xlsx-driven copy swaps onto the Home, Regulations, and Methodology
pages. Subsequent user stories will modify v29 with article-link audit
...
Run:
    python3 -m pytest test_lexicon_v29.py -q
or standalone:
    python3 test_lexicon_v29.py
"""
...
HTML = HERE / "digital_lexicon_v29.html"
...
def test_home_text_from_xlsx():
    ...
    # from the Regulations page). After v29 added the AB 2013 card to the
...
#      the methodology description and was removed in v29.                    #
# --------------------------------------------------------------------------- #
...
def _lr_provider_section(html: str) -> str:
    ...
    minified CONCEPTS literal, the same way build_v29's
...
# Citation patterns lifted from build_v29's audit logic.
_CITE_RE = {
...
# v29 ships two distinct exponent values, both linked to FLOPs thresholds:
...
def test_known_exponents_render_as_superscript():
    ...
    # The 10^26 value must appear in BOTH forms in v29: static <sup>
...
#     both are empty.
#
# A "broken pop-up" in v29 means the drawer body is "No text available."
...
# --------------------------------------------------------------------------- #
# T13. US-010 — every regulatory-text law-blob in v29 is structurally well-  #
...
def _iter_law_blobs(html: str):
    ...
    while True:
        ...
        # Skip the placeholder template comment block left by build_v29.
        try:
...
def test_v29_bill_label_helpers_present():
    """Matrix renderer must use the bill-bracket helpers introduced in v29."""
    html = _html()
    assert "function _v29FormatBill" in html, (
        "v29 bill formatter missing — concepts matrix won't render '(SB 24-205)'."
    )
    assert "function _v29CellLabel" in html, "v29 cell-label helper missing."
    assert "if (v.bill && j !== 'eu')" in html, (
        "v29 cluster-matrix rendering does not skip bill brackets for the EU column."
    )


def test_v29_status_pills_removed_from_regulations():
    """Regulations cards must not carry interpretive/in-effect/amendment pills."""
    html = _html()
    assert '<span class="status-pill' not in html, (
        "v29 still has <span class='status-pill ...'> labels — they should be stripped."
    )
    cards = re.findall(r'<article class="law-card-v2[^"]*">.*?</article>',
                       html, re.DOTALL)
    for card in cards:
        assert "status-pill" not in card, (
            f"law-card-v2 still references status-pill class: {card[:200]}"
        )


def test_v29_drawer_renders_articles_not_verbatim():
    """The drawer wrapper must be the v29 article-rendering one."""
    html = _html()
    assert "__v29_udc_patched" in html, (
        "v29 drawer wrapper flag missing — drawer still renders verbatim text."
    )
    assert "function _renderDrawerArticles" in html, (
        "v29 _renderDrawerArticles function not present."
    )
    assert "Explore in full law" not in html, (
        "v29 still has the 'Explore in full law' button — it should be removed."
    )
    assert ">From the law<" in html, (
        "v29 drawer label should read 'From the law' (was 'Verbatim citation')."
    )


def test_v29_ref_map_has_bare_anchors():
    """REF_MAP must use bare article/section IDs (e.g. '3' not '3-3')."""
    html = _html()
    m = re.search(r"window\.REF_MAP\s*=\s*(\{.*?\});", html, re.DOTALL)
    assert m, "window.REF_MAP not found in v29"
    ref_map = json.loads(m.group(1))
    assert len(ref_map) >= 180, f"v29 REF_MAP unexpectedly small: {len(ref_map)} entries"

    art33 = ref_map.get("EU AI Act, Article 3 (3)")
    assert art33 is not None, "Expected REF_MAP key 'EU AI Act, Article 3 (3)' missing"
    assert art33.get("anchor") == "3", (
        f"v29 should use bare article-id anchors; got {art33.get('anchor')!r}"
    )
    s1428 = ref_map.get("NY S8828 §1428")
    assert s1428 is not None, "Expected REF_MAP key 'NY S8828 §1428' missing"
    assert s1428.get("law") == "ny-s8828"
    assert s1428.get("anchor") == "1428"


def test_v29_eu_ai_act_blob_has_annexes_and_full_recitals():
    """The eu-ai-act law-blob must contain all 13 annexes and 180 recitals."""
    html = _html()
    m = re.search(
        r'<script type="application/json" id="law-blob-eu-ai-act">(.*?)</script>',
        html, re.DOTALL,
    )
    assert m, "law-blob-eu-ai-act not found"
    blob = json.loads(m.group(1))
    annexes = blob.get("annexes")
    assert isinstance(annexes, list) and len(annexes) >= 13, (
        f"eu-ai-act annexes incomplete: {len(annexes) if annexes else 0}"
    )
    annex_ids = {a.get("id") for a in annexes}
    for cited in ("III", "IV", "XI", "XII"):
        assert cited in annex_ids, f"Cited annex {cited} missing from eu-ai-act blob"
    recitals = blob.get("recitals", {})
    assert isinstance(recitals, dict) and len(recitals) >= 180, (
        f"eu-ai-act recitals incomplete: {len(recitals)} entries"
    )
    assert "100" in recitals, "Recital (100) cited in concepts but missing from blob"


def test_v29_drawer_body_formatter_present():
    """The drawer must build paragraphs via _formatBody (no flush pre-wrap)."""
    html = _html()
    assert "function _formatBody" in html, (
        "v29 _formatBody helper missing — drawer body won't indent (a)/(b) sub-points."
    )
    assert "_formatBody(String(art.text" in html, (
        "v29 article body still uses _escape directly; should use _formatBody."
    )
    # Ensure the CSS classes for nested levels exist.
    for cls in (".v29-p0", ".v29-p1", ".v29-p2"):
        assert cls in html, f"v29 nested-paragraph CSS class {cls} missing"


def test_v29_concept_subtitles_show_bill():
    """Non-EU column subtitles must use bills (formatted) instead of repeating
    the column name."""
    html = _html()
    # The renderAnalysisTable header must branch on j === 'eu' to choose
    # bills vs law for the .j-law subtitle.
    assert "j === 'eu'" in html and "jd.bills" in html, (
        "v29 column-header subtitle does not branch on jurisdiction to "
        "show jd.bills for non-EU columns."
    )
    # Ensure all four wrapper sites were updated (search for the SB/HB/AB
    # space-normaliser pattern).
    pattern = "replace(/^(SB|HB|AB)(?=[0-9])/, '$1 ')"
    occurrences = html.count(pattern)
    # There are 4 column-header sites (original + 3 wrappers) plus the
    # _v29FormatBill helper used by the matrix renderer = 5 expected.
    assert occurrences >= 4, (
        f"Expected >=4 occurrences of the bill-formatter pattern, found {occurrences}"
    )


def test_v29_ny_s8828_blob_has_full_sections():
    """ny-s8828 blob must include §1420-1429 (was §1420-1425 in v28)."""
    html = _html()
    m = re.search(
        r'<script type="application/json" id="law-blob-ny-s8828">(.*?)</script>',
        html, re.DOTALL,
    )
    assert m, "law-blob-ny-s8828 not found"
    blob = json.loads(m.group(1))
    section_ids = {s.get("id") for s in blob.get("sections", [])}
    for required in ("1420", "1425", "1426", "1427", "1428", "1429"):
        assert required in section_ids, f"NY S8828 §{required} missing from blob"
